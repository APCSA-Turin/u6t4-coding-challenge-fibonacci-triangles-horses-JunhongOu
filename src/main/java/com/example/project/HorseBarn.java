...
public class HorseBarn {
    ...
    public String horseBarnInfo() {
        StringBuilder barnInfo = new StringBuilder();
        for (int i = 0; i < stalls.length; i++){
            barnInfo.append("Stall ").append(i).append(": ");
            if (stalls[i] == null){
                barnInfo.append("empty");
            }
            else{
                barnInfo.append(stalls[i].horseInfo());
            }
            barnInfo.append("\n");
        }
        return barnInfo.toString();
    }
    ...
    public void consolidate() {
        Horse[] consolidatedStalls = new Horse[stalls.length];
        int index = 0;

        for (Horse horse : stalls){
            if (horse != null){
                consolidatedStalls[index] = horse;
                index++;
            }
        }
        stalls = consolidatedStalls;
    }
}
